...
const FilterSelect: React.FC<Props> = (props: Props) => {
  ...
  return (
    <div
      className={`flex flex-col col-span-6 items-start max-w-[full] gap-2 ${className}`}
    >
      <Label className="text-[#03050F] font-lato text-[16px] font-normal tracking-[-0.64]">
        {label}
      </Label>
        ...
        <Controller
          ...
          render={({ field }) => (
            <Select
              ...
              // value={field.value || ""}
            >
              <SelectTrigger className="w-full h-auto! p-5 shadow-none outline-none! bg-[#FBFBFE] text-[16px] !ring-0! ring-offset-0! focus:ring-0! focus:ring-offset-0! focus:outline-none border border-black/20">
                <SelectValue
